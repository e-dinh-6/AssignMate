...
describe("getUser function", () => {
  ...
  test("Testing getUser function w/ username", () => {
    jest
      .spyOn(mut, "findUserByName")
      .mockResolvedValue([{ username: "User1" }]);
...
/* describe("findUserByUsernameAndPassword function", () => {
  test("should return user based on username and password", () => {
    jest
      .spyOn(User, "find")
      .mockResolvedValue([{ username: "User1", password: "password" }]);
    const expected = [{ username: "User1", password: "password" }];
    return mut
      .findUserByUsernameAndPassword("User1", "password")
      .then((got) => {
        expect(got).toEqual(expected);
      });
  });
